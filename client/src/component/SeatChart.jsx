...
const SeatChart = ({occasion, state, setToggle}) => {
    ...
    const buyHandler = async(_seat)=>{
        setHasSold(false)
        const tx = await contract.mint(occasion.id, _seat, {value:occasion.cost})
        await tx.wait()
        setHasSold(true)
  ...
  return (
    ...
    <div className="occasion__seating">
      ...
      {seatsTaken.length>0 && Array(Math.max(0,Number(occasion.maxTickets) -50)).fill(1).map((e, i) =>
        <Seat
          i={i}
          step={26}
          columnStart={6}
      ...
      {seatsTaken.length>0 && Array(25).fill(1).map((e, i) =>
        <Seat
          i={i}
